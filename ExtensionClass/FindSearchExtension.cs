...
if (tmpDis<radius)
{
  targetObj.Add(obs);
    ...
    public static T[] RadiusSearch<T>(this UnityEngine.GameObject nowObj,float radius)where T:UnityEngine.Component{
        float tmpDis = 0;           //距離用一時変数
          //string nearObjName = "";    //オブジェクト名称
        List<T> targetObj = null; //オブジェクト

        //タグ指定されたオブジェクトを配列で.取得する
        foreach (UnityEngine.GameObject obs in UnityEngine.Object.FindObjectsOfType(typeof(UnityEngine.GameObject))){
            //自身と取得したオブジェクトの距離を取得
            tmpDis =(obs.transform.position- nowObj.transform.position).sqrMagnitude;
