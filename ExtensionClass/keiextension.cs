...
return im;
  ...
  public static GameObject CreateMesImage(this string st,GameObject obj,Vector3 posi,float duration=3){
var im=("CustomTextImage").ResourcesLoad();
im= im.Instantiate(obj.transform);
im.gameObject.AddComponentIfnull<UIcameralook>();
im.transform.GetChild(0).GetComponent<Text>().DOText(st,1f).OnComplete(()=>{keikei.delaycall(()=>im.destroy(),duration);});
im.transform.parent=obj.transform;
...
public static void addforce(this MonoBehaviour mono, GameObject obj,int forcepowers,Transform trans){
  ...
  mono.StartCoroutine(obj.transform.AddForces((obj.transform.position-trans.position)*forcepowers));     
}
public static void PlayerAddForce(this GameObject obj,Vector3 force){
obj.root().GetComponent<UnityChanControlScriptWithRgidBody>().AddForce(force);
}
