...
void FixedUpdate()
{
    ...
    {
        time+=Time.deltaTime;
        timetext.text="経過時間:"+time.ToString("F2")+"残り"+(problemcount-rightanswercount).ToString()+"問";
    }else
    {
        time=0;
    }
...
marubasu.text="ゲームオーバー。。 かかった時間："+((int)time).ToString()+"成績："+(10000/(rightanswercount)*(int)time).ToString();
...
marubasu.text="ステージクリア！！ かかった時間："+((int)time).ToString()+"成績："+(10000/(rightanswercount)*(int)time).ToString();
reset();
    }
